...
Lg_tree <- plots_f_match %>%
  #filter only records matching plots_f_match 
  inner_join(Large_tree, 
...
#Lg_tree_sub <- Lg_tree %>%
#  select(S_Year, S_Cycle, Unit_Code, Sampling_Frame, Plot_Type, 
#         Plot_Number, Quad, Name, Code, Life_Form, Nativity, 
#         Status, Vigor, DBH) %>%
#  right_join(plots_f_match, by = c("Unit_Code", "Sampling_Frame", "Plot_Number")) #%>%
#  #filter(!is.na(DBH)) %>%


#write_csv(Lg_tree_sub, here("tbls", "Lg_tree_subset.csv"))
...
Lg_trees_pp <- Lg_tree %>%
  ...
  filter(Code != "HIBTIL") %>%
  group_by(S_Cycle, Unit_Code, Sampling_Frame, Plot_Number, Status) %>%
  summarise(Trees_pp = n()) 
