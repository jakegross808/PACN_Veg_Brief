...
#  ........ BAR MEANS----
central_plots <- c("2", "4", "6", "9")
outer_plots <- c("1","3","5","7","8","10")

weed_cover_central <- Forms_Cov_1a2 %>%
  filter(Plot_Number %in% central_plots) %>%  
  group_by(S_Cycle, Unit_Code, Sampling_Frame, Nativity, Life_form, NLF) %>%
  summarise(mean_NLF = mean(tot_pct_cov),
            max_NLF = max(tot_pct_cov),
            min_NLF = min(tot_pct_cov)) 

weed_cover_outer <- Forms_Cov_1a2 %>%
  filter(Plot_Number %in% outer_plots) %>%
  group_by(S_Cycle, Unit_Code, Sampling_Frame, Nativity, Life_form, NLF) %>%
  summarise(mean_NFL = mean(tot_pct_cov),
            max_NLF = max(tot_pct_cov),
            min_NLF = min(tot_pct_cov)) 
  ...
  ggplot(aes(x = reorder(NLF, mean_NFL), y = mean_NFL, fill = Nativity)) +
  geom_bar(colour = "black", stat="identity", alpha = 0.5) 
  
  
  #filter(NLF %in% fcs.NLF_1a2) %>%
  ggplot(aes(x = reorder(NLF, mean_NFL), y = mean_NFL, fill = Nativity)) +
  #geom_linerange(data = Forms_Cov_1a2, aes(ymin = MEAN - ERR, ymax = MEAN + ERR)) +
  #geom_point(
...
target_plots <- c("1","3","5","7","8","10")
Forms_Cov_Chg_1a2 <- Forms_Cov_Complete_1a2 %>%
  pivot_wider(names_from = S_Cycle, values_from = tot_pct_cov) %>%
  mutate(tot_pct_cov_chg = round(`2` - `1`, 2))  %>%
  filter(Plot_Number %in% target_plots) 
